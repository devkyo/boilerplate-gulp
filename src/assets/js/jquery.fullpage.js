...
(function(global, factory) {
    ...
    $.fn.fullpage = function(options) {
        ...
        */
        ...
        //Custom Code
        function setColor(){
            if($('.section.active').find('.section-stack.active').hasClass('change')){
                $('.navbar-brand').addClass('grey')
            } else {
                $('.navbar-brand').removeClass('grey')
            }
            var color = $('.section.active').find('.section-stack.active').data('color')
            if (typeof color !== 'undefined'){
                $('.indicators').attr('class', 'indicators '+color)
            }
        }

        var canScrollNow = true;
        function showStackedSection(direction, section){
            if(!canScrollNow) return;
            canScrollNow = false;

            var stacked = $(SECTION_ACTIVE_SEL).find('.section-stack')
            var stackedActive = $(SECTION_ACTIVE_SEL).find('.section-stack.active')
            //direction{true} = up
            if(direction){
                if(stackedActive.index()>0){
                    stackedActive.removeClass('active down').addClass('prev-active')
                    stackedActive.prev().removeClass('prev-active').addClass('active')
                    setTimeout(function(){
                        canScrollNow = true
                    },700)
                } else {
                ...
                if($('body').hasClass('fp-viewing-0') && (stackedActive.index()-1)==0) $('.scroll-navigate').hide()
                setTimeout(function(){
                    $('#opening-03-title').removeAttr('style')
                },400)
            } else {
                if(stackedActive.index()<(stacked.length-1)){
                    stackedActive.removeClass('active').addClass('prev-active')
                    stackedActive.next().removeClass('prev-active').addClass('active')
                    setTimeout(function(){
                        canScrollNow = true
                    },700)
                } else {
                    canScrollNow = true;
                    stackedActive.addClass('down')
                    if (section.length) {
                        scrollPage(section, null, false);
                    }
                }
                if($('body').hasClass('fp-viewing-0') && (stackedActive.index()+1)>0) $('.scroll-navigate').show()
                setTimeout(function(){
                    $('#opening-03-title').removeAttr('style')
                },400)
            }
        ...
        $('.scroll-navigate .icon-back').click(function(){
            moveSectionDown()
        })
        $('.scroll-navigate .icon-forward').click(function(){
            moveSectionUp()
        })
        $('.situation-change ul>li>a').click(function(e){
            e.preventDefault()
        ...
        */
        function moveSectionUp(){
            ...
            //Custom Code
            if($(SECTION_ACTIVE_SEL).hasClass('hold-section')){
                showStackedSection(true, prev)
            } else {
                if (prev.length) {
        ...
        */
        function moveTo(sectionAnchor, slideAnchor){
            ...
            if (typeof slideAnchor !== 'undefined'){
                ...
                for(var i=$('#'+destiny[0].id).index();i<($('.section').length-1);i++){
                    $('#trade0'+i).find('.change').removeClass('prev-active active down')
                    $('#trade0'+i).find('.current').removeClass('prev-active').addClass('active')
                }
                scrollPage(destiny);
                setColor();
            }
